...
if __name__ == '__main__':
    ...
    for batch, (X, y) in enumerate(loader):
        ...
        for k in my_keys:
            while label_index[k] < len(label_dict[k]):
                ...
                if len(label_dict[k]) > label_index[k]:
                ...
                if random.random() < .5:
                    if not (len(label_dict[k]) > label_index[k] + 1):
                        break
                    new_tensors.append(label_dict[k][label_index[k]+1])
                    label_index[k] += 2
                    label_index[available_others[randint]] += 1
                    new_y[0] = 1.0  # First one is repeated
                else:
                    if not (len(label_dict[k]) > label_index[chosen] + 1):
                        continue
                    new_tensors.append(label_dict[chosen][label_index[chosen]+1])
                    new_y[1] = 1.0
                    label_index[k] += 1
                    label_index[chosen] += 2
    ...
    for i, (X, y) in enumerate(new_data):
        ...
        save_image(X[2], os.path.join(directory, str(i), 'img3.png'))
    ...
    pickle.dump({'len': len(new_data), 'desc': 'X contains tensors of three images. y is tensor [1.0, 0.] if first matches third. Otherwise [0., 1.]'}, header_file)
    print("Done.")
